...
import {
  ClickCapture,
  ClickCaptureID,
} from '../controllers/click-capture/click-capture';
...
export class InteractionService {
  ...
  private _selectionChange: BehaviorSubject<Interactor | undefined> =
    new BehaviorSubject<Interactor | undefined>(this.lastSelected);
  ...
  private deleteCurrentSelection() {
    this.selected.forEach((iter) => {
      const iterator = iter.specifyContextMenu();
      let option = iterator[iterator.length - 1];
      option.action();
      console.log(option);
    });
  }
  ...
  private selectNewObject(
    object: Interactor,
    deselectOldObjects: boolean = true
  ): void {
    ...
    this.selected.forEach((oldObj) => {
      if (
        oldObj !== object &&
        (deselectOldObjects || oldObj instanceof SvgInteractor)
      ) {
  ...
  public _onMouseDown(object: Interactor, event: MouseEvent): void {
    ...
    this.selectNewObject(
      object,
      (!this.isPressingKey('Shift') && !alreadySelected) ||
        object instanceof SvgInteractor
    );
  ...
  public _onMouseMove(object: Interactor, event: MouseEvent): void {
    ...
    this.mousePos = {
      ...
      model: this.unitConverter.mouseCoordToModelCoord(
        screenPos,
        currentZoomPan
      ),
    };
